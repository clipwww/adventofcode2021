function day8() {
  ...
  const puzzle3 = `acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf`
  const puzzle2 = `be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce`
  const puzzle = `fgcae ebafc cabdef eg abecfg abgfed feg gafdc bceg ebgcadf | defagbc faecg cfdag gecb
...
bcg bc gdcbae dbca abfgde gdeba ebfacdg egcba faceg dgfbce | geadb badecg ecbfgd baecg`
  ...
  const mapping = [2, 3, 4, 7];
  function part1() {

    const ret = puzzleArr.reduce((sum, line) => {
      const [before, after] = line.split(' | ')
      return sum + after.split(' ').map(str => str.length).filter(len => mapping.includes(len)).length;
    }, 0)
  ...
  function part2() {
    const ret = puzzleArr.reduce((sum, line) => {
      ...
      // find 6 找出 B, C
      const [a, b] = one;
      map.get(6).forEach(str => {
        switch (true) {
          case str.includes(a) && !str.includes(b):
            correct.set('C', a)
            correct.set('B', b)
            six = str;
            break;
          case str.includes(b) && !str.includes(a):
            correct.set('C', b)
            correct.set('B', a)
            six = str;
            break;
      ...
      // find 2, 3, 5
      const B = correct.get('B');
      const C = correct.get('C');

      map.get(5).forEach(str => {
        switch (true) {
          case str.includes(B) && !str.includes(C):
            two = str;
            break;
          case str.includes(C) && !str.includes(B):
            five = str;
            break;
          default:
            three = str;
      ...
      // 比對 2, 3 的不同處找出 E
      two.split('').forEach(letter => {
        if (!three.includes(letter)) {
          correct.set('E', letter);
        }
      })

      // 比對 5, 2 的未知不同處找出 F
      five.split('').forEach(letter => {
        if (letter !== correct.get('C') && !two.includes(letter)) {
          correct.set('F', letter);
        }
      })

      // find 0, 9
      const BCFE = [correct.get('B'), correct.get('C'), correct.get('F'), correct.get('E')];
      map.get(6).forEach(str => {
        if (str === six) return;
        if (BCFE.every(letter => str.includes(letter))) {
          zero = str
        } else {
          nine = str;
        }
      ...
      // 比對 0, 9 剩餘未知的不同處找出 G
      nine.split('').forEach(letter => {
        if (!zero.includes(letter)) {
          correct.set('G', letter);
        }
      })
      
      // 比對 0, 7 剩餘未知的不同處找出 A, D
      zero.split('').forEach(letter => {
        if (!BCFE.includes(letter)) {
          if (seven.includes(letter)) {
            correct.set('A', letter);
          } else {
            correct.set('D', letter);
          }
        }
      })

      // console.log(correct);
      const newSevenSegmentDisplays = sevenSegmentDisplays.map(str => str.split('').map(letter => correct.get(letter)).sort().join(''));
      // console.log(newSevenSegmentDisplays);

      return sum + +after.split(' ')
      .map(str => str.split('').sort().join(''))
      .map(str => newSevenSegmentDisplays.findIndex(nssd => nssd === str)).join('')
    }, 0)
